...
function PropsSI(fluid::AbstractString, output::AbstractString)
    val = ccall( (:Props1SI, libcoolprop), Cdouble, (Cstring, Cstring), fluid, output)
    if val == Inf
...
function set_departure_functions(string_data::AbstractString)
  errcode = ref{Clong}(0);
  ccall( (:set_departure_functions, libcoolprop), Nothing, (Cstring, Ptr{Clong}, Ptr{UInt8}, Int), string_data, errcode, message_buffer::Array{UInt8, 1}, buffer_length)
  if errcode != 0
...
function set_reference_state(fluid::AbstractString, reference_state::AbstractString)
    val = ccall( (:set_reference_stateS, libcoolprop), Cint, (Cstring, Cstring), fluid, reference_state)
    if val == 0
        error("CoolProp: ", get_global_param_string("errstring"))
    end
    return val
...
function set_reference_state(fluid::AbstractString, T::Real, rhomolar::Real, hmolar0::Real, smolar0::Real)
    val = ccall( (:set_reference_stateD, libcoolprop), Cint, (Cstring, Cdouble, Cdouble, Cdouble, Cdouble), fluid, T, rhomolar, hmolar0, smolar0)
    if val == 0
        error("CoolProp: ", get_global_param_string("errstring"))
    end
    return val
...
function saturation_ancillary(fluid_name::AbstractString, output::AbstractString, quality::Integer, input::AbstractString, value::Real)
    val = ccall( (:saturation_ancillary, libcoolprop), Cdouble, (Cstring, Cstring, Cint, Cstring, Cdouble), fluid_name, output, quality, input, value)
    if val == Inf
        error("CoolProp: ", get_global_param_string("errstring"))
    end
    return val
...
function set_config(key::AbstractString, val::AbstractString)
    ccall( (:set_config_string, libcoolprop), Nothing, (Cstring, Cstring), key, val)
...
function set_config(key::AbstractString, val::Real)
    ccall( (:set_config_double, libcoolprop), Nothing, (Cstring, Cdouble), key, val)
    return get_global_param_string("errstring")
end
...
function set_config(key::AbstractString, val::Bool)
    ccall( (:set_config_bool, libcoolprop), Nothing, (Cstring, UInt8), key, val)
    return get_global_param_string("errstring")
end
...
function F2K(tf::Real)
    return ccall( (:F2K, libcoolprop), Cdouble, (Cdouble,), tf)
end
...
function K2F(tk::Real)
    return ccall( (:K2F, libcoolprop), Cdouble, (Cdouble,), tk)
end
...
function get_param_index(param::AbstractString)
    val = ccall( (:get_param_index, libcoolprop), Clong, (Cstring,), param)
    if val == -1
...
function get_input_pair_index(pair::AbstractString)
    val = ccall( (:get_input_pair_index, libcoolprop), Clong, (Cstring,), pair)
    if val == -1
...
function get_debug_level()
    ccall( (:get_debug_level, libcoolprop), Cint, () )
end
...
function set_debug_level(level::Integer) # change ::Int to ::Integer to make set_debug_level(get_debug_level()) works on different machine
    ccall( (:set_debug_level, libcoolprop), Nothing, (Cint,), level)
end
...
function HAPropsSI(output::AbstractString, name1::AbstractString, value1::Real, name2::AbstractString, value2::Real, name3::AbstractString, value3::Real)
    val = ccall( (:HAPropsSI, libcoolprop), Cdouble, (Cstring, Cstring, Cdouble, Cstring, Cdouble, Cstring, Cdouble), output, name1, value1, name2, value2, name3, value3)
    if val == Inf
        error("CoolProp: ", get_global_param_string("errstring"))
    end
    return val
...
function cair_sat(t::Real)
    val = ccall( (:cair_sat, libcoolprop), Cdouble, (Cdouble, ), t)
    return val;
end
